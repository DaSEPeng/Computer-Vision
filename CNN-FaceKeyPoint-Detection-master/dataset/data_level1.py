...
将提供的数据集转换成level1可用的数据格式，并进行反转、旋转等数据增强
...
def generate_hdf5_data(filelist, output, fname, augment=False):
    ...
    for (imgPath, bbox, landmarkGt) in data:    
        ...
        ## data augment
        if augment and np.random.rand() > -1:
            ### flip
...
if __name__ == '__main__':
    ...
    generate_hdf5_data(train_txt, OUTPUT, 'train.h5', augment=True)
